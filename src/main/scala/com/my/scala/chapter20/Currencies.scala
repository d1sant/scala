...
object Currencies {
  ...
  abstract class FaultyCurrency {
    ...
    // def + (that: FaultyCurrency): FaultyCurrency = ...
    // def * (x: Double): FaultyCurrency = ...
  }
  abstract class Dollar extends FaultyCurrency {
    def designation = "USD"
  }
  abstract class Euro extends FaultyCurrency {
    def designation = "Euro"
  }
  
  // A second (still imperfect) design of the Currency class
  abstract class ImperfectAbstractCurrency {
    type Currency <: ImperfectAbstractCurrency
    val amount: Long
    def designation: String
    override def toString = amount + " " + designation
    // def + (that: Currency): Currency = new Currency { // throws an error class type required
    //   val amount = this.amount + that.amount
    // }
    // def * (x: Double): Currency = ...
    def make(amount: Long): Currency // factory method 
  }
  abstract class Dollar2 extends ImperfectAbstractCurrency {
    type Currency = Dollar2
    def designation = "USD"
  }
